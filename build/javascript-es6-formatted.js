  ...
  globalThis.Element = class extends globalThis.Node {
    ...
    get innerHTML() {
      return (
        this._innerHTML ||
        `<${this.tagName}>\n${this.childNodes
          ?.map((n) => (" " + (n.innerHTML || "#text")).replaceAll("\n", "\n "))
          .join("\n")}\n</${this.tagName}>`
      );
    }
    get attributes() {
    ...
    querySelectorAll(sel) {
      if (sel[0] == "." && sel.indexOf(" ") == -1) {
        return Array.prototype.slice.call(
          this.getElementsByClassName(sel.substring(1))
        );
      }
        ...
        for (var a of Array.prototype.slice.call(ancestors)) {
          for (var c of a.childNodes) {
            result = result.concat(
              makeArrayLike(
                new HTMLCollection(document, this, (node) => node.href == "#/")
              )
            );
          }
        }
...
sectionTodoApp.appendChild(clearCompleted); /* eslint no-unused-vars: 0 */
...
class Store {
  constructor(name, callback) {
    ...
    if (callback) callback.call(this, JSON.parse(this.memoryStorage[name]));
  }
...
class Controller {
  ...
  toggleAll(completed) {
    this.model.read({ completed: !completed }, (data) => {
      for (let item of data) this.toggleComplete(item.id, completed, true);
    });
