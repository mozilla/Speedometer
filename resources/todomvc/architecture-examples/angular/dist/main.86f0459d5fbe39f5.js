...
(self.webpackChunkangular = self.webpackChunkangular || []).push([ [ 179 ], {
    826: () => {
        ...
        function readableStreamLikeToAsyncGenerator(readableStream) {
            return function __asyncGenerator(thisArg, _arguments, generator) {
                if (!Symbol.asyncIterator) throw new TypeError("Symbol.asyncIterator is not defined.");
        ...
        function fesm2020_core_inject(token, flags = InjectFlags.Default) {
            return core_inject(token, convertToBitFlags(flags));
        }
        function convertToBitFlags(flags) {
            return typeof flags > "u" || "number" == typeof flags ? flags : 0 | (flags.optional && 8) | (flags.host && 1) | (flags.self && 2) | (flags.skipSelf && 4);
        }
            ...
            for (let i = 0; i < types.length; i++) {
                ...
                if (Array.isArray(arg)) {
                    ...
                    for (let j = 0; j < arg.length; j++) {
                        const meta = arg[j], flag = meta[DI_DECORATOR_FLAG];
        ...
        function attachInjectFlag(decorator, flag) {
            return decorator[DI_DECORATOR_FLAG] = flag, decorator.prototype[DI_DECORATOR_FLAG] = flag, 
            decorator;
        }
        ...
        function ɵɵdefineComponent(componentDefinition) {
            return noSideEffects(() => {
                ...
                return def.directiveDefs = extractDefListOrFactory(dependencies, !1), def.pipeDefs = extractDefListOrFactory(dependencies, !0), 
                def;
            });
        }
        ...
        function getNgDirectiveDef(directiveDefinition) {
            ...
            return {
                ...
                standalone: !0 === directiveDefinition.standalone,
        ...
        function extractDefListOrFactory(dependencies, pipeDef) {
            ...
            return () => ("function" == typeof dependencies ? dependencies() : dependencies).map(dep => defExtractor(dep)).filter(nonNull);
        }
        const HOST = 0, TVIEW = 1, FLAGS = 2, PARENT = 3, NEXT = 4, TRANSPLANTED_VIEWS_TO_REFRESH = 5, T_HOST = 6, CLEANUP = 7, CONTEXT = 8, INJECTOR$1 = 9, RENDERER_FACTORY = 10, RENDERER = 11, SANITIZER = 12, CHILD_HEAD = 13, CHILD_TAIL = 14, DECLARATION_VIEW = 15, DECLARATION_COMPONENT_VIEW = 16, DECLARATION_LCONTAINER = 17, PREORDER_HOOK_FLAGS = 18, QUERIES = 19, ID = 20, EMBEDDED_VIEW_INJECTOR = 21, HEADER_OFFSET = 22, TYPE = 1, HAS_TRANSPLANTED_VIEWS = 2, NATIVE = 7, VIEW_REFS = 8, MOVED_VIEWS = 9, CONTAINER_HEADER_OFFSET = 10;
        ...
        function ngOnChangesSetInput(instance, value, publicName, privateName) {
            const declaredName = this.declaredInputs[publicName], simpleChangesStore = getSimpleChangesStore(instance) || function setSimpleChangesStore(instance, store2) {
                return instance[SIMPLE_CHANGES_STORE] = store2;
            }(instance, {
                previous: EMPTY_OBJ,
                current: null
            }), current = simpleChangesStore.current || (simpleChangesStore.current = {}), previous = simpleChangesStore.previous, previousChange = previous[declaredName];
        ...
        function updateTransplantedViewCount(lContainer, amount) {
            ...
            let viewOrContainer = lContainer, parent = lContainer[PARENT];
            for (;null !== parent && (1 === amount && 1 === viewOrContainer[TRANSPLANTED_VIEWS_TO_REFRESH] || -1 === amount && 0 === viewOrContainer[TRANSPLANTED_VIEWS_TO_REFRESH]); ) parent[TRANSPLANTED_VIEWS_TO_REFRESH] += amount, 
            viewOrContainer = parent, parent = parent[PARENT];
        }
        ...
        function getDeclarationTNode(lView) {
            ...
            return 2 === tView.type ? tView.declTNode : 1 === tView.type ? lView[T_HOST] : null;
        }
        function enterDI(lView, tNode, flags) {
            if (flags & InjectFlags.SkipSelf) {
                ...
                null === parentTNode || (parentLView = parentLView[DECLARATION_VIEW], 10 & parentTNode.type))); ) ;
        ...
        function enterView(newView) {
            const newLFrame = allocLFrame(), tView = newView[TVIEW];
        ...
        function incrementInitPhaseFlags(lView, initPhase) {
            ...
            (3 & flags) === initPhase && (flags &= 2047, flags += 1, lView[FLAGS] = flags);
        }
        function callHooks(currentView, arr, initPhase, currentNodeIndex) {
            const nodeIndexLimit = currentNodeIndex ?? -1, max = arr.length - 1;
            let lastNodeIndexFound = 0;
            for (let i = void 0 !== currentNodeIndex ? 65535 & currentView[PREORDER_HOOK_FLAGS] : 0; i < max; i++) if ("number" == typeof arr[i + 1]) {
        ...
        function callHook(currentView, initPhase, arr, i) {
            ...
            if (isInitHook) {
                if (currentView[FLAGS] >> 11 < currentView[PREORDER_HOOK_FLAGS] >> 16 && (3 & currentView[FLAGS]) === initPhase) {
                    currentView[FLAGS] += 2048, profiler(4, directive, hook);
                    try {
                        hook.call(directive);
                    } finally {
        ...
        const BLOOM_MASK = 255, BLOOM_BUCKET_BITS = 5;
        ...
        function getOrCreateNodeInjectorForNode(tNode, lView) {
            ...
            if (hasParentInjector(parentLoc)) {
                const parentIndex = getParentInjectorIndex(parentLoc), parentLView = getParentInjectorView(parentLoc, lView), parentData = parentLView[TVIEW].data;
        ...
        function getParentInjectorLocation(tNode, lView) {
            ...
            for (;null !== lViewCursor; ) {
                if (parentTNode = getTNodeFromLView(lViewCursor), null === parentTNode) return NO_PARENT_INJECTOR;
                if (declarationViewOffset++, lViewCursor = lViewCursor[DECLARATION_VIEW], -1 !== parentTNode.injectorIndex) return parentTNode.injectorIndex | declarationViewOffset << 16;
            }
        ...
        function diPublicInInjector(injectorIndex, tView, token) {
            !function bloomAdd(injectorIndex, tView, type) {
                ...
                tView.data[injectorIndex + (bloomHash >> BLOOM_BUCKET_BITS)] |= 1 << bloomHash;
        ...
        function lookupTokenUsingModuleInjector(lView, token, flags, notFoundValue) {
            ...
            !(flags & (InjectFlags.Self | InjectFlags.Host))) {
            ...
            if (null !== tNode) {
                if (1024 & lView[FLAGS]) {
                    const embeddedInjectorValue = function lookupTokenUsingEmbeddedInjector(tNode, lView, token, flags, notFoundValue) {
                        ...
                        for (;null !== currentTNode && null !== currentLView && 1024 & currentLView[FLAGS] && !(256 & currentLView[FLAGS]); ) {
                            ...
                            if (!parentTNode) {
                                ...
                                parentTNode = getTNodeFromLView(currentLView), currentLView = currentLView[DECLARATION_VIEW];
                            }
        ...
        function lookupTokenUsingNodeInjector(tNode, lView, token, flags, notFoundValue) {
            const bloomHash = function bloomHashBitOrFactory(token) {
                ...
                return "number" == typeof tokenId ? tokenId >= 0 ? tokenId & BLOOM_MASK : createNodeInjector : tokenId;
            }(token);
            if ("function" == typeof bloomHash) {
                ...
                let previousTView = null, injectorIndex = getInjectorIndex(tNode, lView), parentLocation = NO_PARENT_INJECTOR, hostTElementNode = flags & InjectFlags.Host ? lView[DECLARATION_COMPONENT_VIEW][T_HOST] : null;
                ...
                parentLocation !== NO_PARENT_INJECTOR && shouldSearchParent(flags, !1) ? (previousTView = lView[TVIEW], 
                injectorIndex = getParentInjectorIndex(parentLocation), lView = getParentInjectorView(parentLocation, lView)) : injectorIndex = -1); -1 !== injectorIndex; ) {
                    ...
                    parentLocation = lView[injectorIndex + 8], parentLocation !== NO_PARENT_INJECTOR && shouldSearchParent(flags, lView[TVIEW].data[injectorIndex + 8] === hostTElementNode) && bloomHasToken(bloomHash, injectorIndex, lView) ? (previousTView = tView, 
        ...
        function searchTokensOnInjector(injectorIndex, lView, token, previousTView, flags, hostTElementNode) {
            const currentTView = lView[TVIEW], tNode = currentTView.data[injectorIndex + 8], injectableIdx = function locateDirectiveOrProvider(tNode, tView, token, canAccessViewProviders, isHostSpecialCase) {
        ...
        function bloomHasToken(bloomHash, injectorIndex, injectorView) {
            return !!(injectorView[injectorIndex + (bloomHash >> BLOOM_BUCKET_BITS)] & 1 << bloomHash);
        }
        ...
        function getTNodeFromLView(lView) {
            const tView = lView[TVIEW], tViewType = tView.type;
            return 2 === tViewType ? tView.declTNode : 1 === tViewType ? lView[T_HOST] : null;
        }
        ...
        function attachPatchData(target, data) {
            isLView(data) ? (target[MONKEY_PATCH_KEY_NAME] = data[ID], function registerLView(lView) {
                TRACKED_LVIEWS.set(lView[ID], lView);
            }(data)) : target[MONKEY_PATCH_KEY_NAME] = data;
        }
        ...
        function getNearestLContainer(viewOrContainer) {
            for (;null !== viewOrContainer && !isLContainer(viewOrContainer); ) viewOrContainer = viewOrContainer[NEXT];
        ...
        function detachMovedView(declarationContainer, lView) {
            const movedViews = declarationContainer[MOVED_VIEWS], declarationViewIndex = movedViews.indexOf(lView), insertionLContainer = lView[PARENT];
            512 & lView[FLAGS] && (lView[FLAGS] &= -513, updateTransplantedViewCount(insertionLContainer, -1)), 
        ...
        function detachView(lContainer, removeIndex) {
            ...
            if (viewToDetach) {
                ...
                !function removeViewFromContainer(tView, lView) {
                    applyView(tView, lView, lView[RENDERER], 2, null, null), lView[HOST] = null, lView[T_HOST] = null;
                }(viewToDetach[TVIEW], viewToDetach);
                const lQueries = removedLView[QUERIES];
                null !== lQueries && lQueries.detachView(removedLView[TVIEW]), viewToDetach[PARENT] = null, 
                viewToDetach[NEXT] = null, viewToDetach[FLAGS] &= -65;
            }
            return viewToDetach;
        }
            ...
            if (!(128 & lView[FLAGS])) {
                ...
                renderer.destroyNode && applyView(tView, lView, renderer, 3, null, null), function destroyViewTree(rootView) {
                    ...
                    for (;lViewOrLContainer; ) {
                        ...
                        if (!next) {
                            for (;lViewOrLContainer && !lViewOrLContainer[NEXT] && lViewOrLContainer !== rootView; ) isLView(lViewOrLContainer) && cleanUpView(lViewOrLContainer[TVIEW], lViewOrLContainer), 
                            lViewOrLContainer = lViewOrLContainer[PARENT];
                            null === lViewOrLContainer && (lViewOrLContainer = rootView), isLView(lViewOrLContainer) && cleanUpView(lViewOrLContainer[TVIEW], lViewOrLContainer), 
        ...
        function cleanUpView(tView, lView) {
            if (!(128 & lView[FLAGS])) {
                lView[FLAGS] &= -65, lView[FLAGS] |= 128, function executeOnDestroys(tView, lView) {
                    let destroyHooks;
                    ...
                    const tCleanup = tView.cleanup, lCleanup = lView[CLEANUP];
                    let lastLCleanupIndex = -1;
                    if (null !== tCleanup) for (let i = 0; i < tCleanup.length - 1; i += 2) if ("string" == typeof tCleanup[i]) {
                        const targetIdx = tCleanup[i + 3];
                        targetIdx >= 0 ? lCleanup[lastLCleanupIndex = targetIdx]() : lCleanup[lastLCleanupIndex = -targetIdx].unsubscribe(), 
                        i += 2;
                    } else {
                ...
                }(tView, lView), 1 === lView[TVIEW].type && lView[RENDERER].destroy();
                const declarationContainer = lView[DECLARATION_LCONTAINER];
                if (null !== declarationContainer && isLContainer(lView[PARENT])) {
                    declarationContainer !== lView[PARENT] && detachMovedView(declarationContainer, lView);
        ...
        function getParentRElement(tView, tNode, lView) {
            return function getClosestRElement(tView, tNode, lView) {
                ...
                if (null === parentTNode) return lView[HOST];
                {
        ...
        let _processI18nInsertBefore, core_DOCUMENT, _getInsertInFrontOfRNodeWithI18n = function getInsertInFrontOfRNodeWithNoI18n(parentTNode, currentTNode, lView) {
            return 40 & parentTNode.type ? getNativeByTNode(parentTNode, lView) : null;
        };
        function appendChild(tView, lView, childRNode, childTNode) {
            const parentRNode = getParentRElement(tView, childTNode, lView), renderer = lView[RENDERER], anchorNode = function getInsertInFrontOfRNode(parentTNode, currentTNode, lView) {
                return _getInsertInFrontOfRNodeWithI18n(parentTNode, currentTNode, lView);
            }(childTNode.parent || lView[T_HOST], childTNode, lView);
            ...
            void 0 !== _processI18nInsertBefore && _processI18nInsertBefore(renderer, childTNode, lView, childRNode, parentRNode);
        }
        function getFirstNativeNode(lView, tNode) {
            if (null !== tNode) {
                ...
                if (32 & tNodeType) return icuContainerIterate(tNode, lView)() || unwrapRNode(lView[tNode.index]);
                {
                    ...
                    return null !== projectionNodes ? Array.isArray(projectionNodes) ? projectionNodes[0] : getFirstNativeNode(getLViewParent(lView[DECLARATION_COMPONENT_VIEW]), projectionNodes) : getFirstNativeNode(lView, tNode.next);
                }
            }
            return null;
        }
        function getProjectionNodes(lView, tNode) {
            return null !== tNode ? lView[DECLARATION_COMPONENT_VIEW][T_HOST].projection[tNode.projection] : null;
        }
        ...
        function applyNodes(renderer, action, tNode, lView, parentRElement, beforeNode, isProjection) {
            for (;null != tNode; ) {
                ...
                tNode.flags |= 2), 32 != (32 & tNode.flags)) if (8 & tNodeType) applyNodes(renderer, action, tNode.child, lView, parentRElement, beforeNode, !1), 
        ...
        function applyProjectionRecursive(renderer, action, lView, tProjectionNode, parentRElement, beforeNode) {
            const componentLView = lView[DECLARATION_COMPONENT_VIEW], nodeToProjectOrRNodes = componentLView[T_HOST].projection[tProjectionNode.projection];
            if (Array.isArray(nodeToProjectOrRNodes)) for (let i = 0; i < nodeToProjectOrRNodes.length; i++) applyToElementOrContainer(action, renderer, parentRElement, nodeToProjectOrRNodes[i], beforeNode); else applyNodes(renderer, action, nodeToProjectOrRNodes, componentLView[PARENT], parentRElement, beforeNode, !0);
        }
        ...
        function setupStaticAttributes(renderer, element, tNode) {
            ...
            null !== mergedAttrs && setUpAttributes(renderer, element, mergedAttrs), null !== classes && writeDirectClass(renderer, element, classes), 
            null !== styles && function writeDirectStyle(renderer, element, newValue) {
                renderer.setAttribute(element, "style", newValue);
            }(renderer, element, styles);
        }
        class SafeValueImpl {
        ...
        function walkProviderTree(container, providersOut, parents, dedup) {
            ...
            if (cmpDef) {
                ...
                if (!injDef) return !1;
                {
                    ...
                    null == defProviders || isDuplicate || deepForEachProvider(defProviders, provider => {
                        providersOut.push(provider);
                    });
                }
            }
        ...
        class R3Injector extends EnvironmentInjector {
            ...
            get(token, notFoundValue = THROW_IF_NOT_FOUND, flags = InjectFlags.Default) {
                this.assertNotDestroyed(), flags = convertToBitFlags(flags);
                const previousInjector = setCurrentInjector(this), previousInjectImplementation = setInjectImplementation(void 0);
                try {
                    if (!(flags & InjectFlags.SkipSelf)) {
                    ...
                    if ("NullInjectorError" === e.name) {
                        ...
                        return function catchInjectorError(e, token, injectorErrorName, source) {
                            ...
                            throw token[SOURCE] && tokenPath.unshift(token[SOURCE]), e.message = function formatError(text, obj, injectorErrorName, source = null) {
                                text = text && "\n" === text.charAt(0) && text.charAt(1) == NO_NEW_LINE ? text.slice(2) : text;
        ...
        function injectableDefOrInjectorDefFactory(token) {
            ...
            if (token instanceof Function) return function getUndecoratedInjectableFactory(token) {
                ...
                const inheritedInjectableDef = function getInheritedInjectableDef(type) {
                    ...
                    return def ? (function getTypeName(type) {
                        if (type.hasOwnProperty("name")) return type.name;
                        ("" + type).match(/^function\s*([^\s(]+)/);
                    }(type), def) : null;
                }(token);
        ...
        const VERSION = new Version("15.2.5"), NOT_FOUND_CHECK_ONLY_ELEMENT_INJECTOR = {}, ERROR_ORIGINAL_ERROR = "ngOriginalError";
        ...
        function isCssClassMatching(attrs, cssClassToMatch, isProjectionMode) {
            let i = 0, isImplicitAttrsSection = !0;
            for (;i < attrs.length; ) {
                let item = attrs[i++];
                if ("string" == typeof item && isImplicitAttrsSection) {
                    const value = attrs[i++];
                    if (isProjectionMode && "class" === item && -1 !== classIndexOf(value.toLowerCase(), cssClassToMatch, 0)) return !0;
                } else {
                    if (1 === item) {
                        for (;i < attrs.length && "string" == typeof (item = attrs[i++]); ) if (item.toLowerCase() === cssClassToMatch) return !0;
                        return !1;
                    }
                    "number" == typeof item && (isImplicitAttrsSection = !1);
                }
            }
            return !1;
        }
        function isInlineTemplate(tNode) {
            return 4 === tNode.type && tNode.value !== NG_TEMPLATE_SELECTOR;
        }
        function hasTagAndTypeMatch(tNode, currentSelector, isProjectionMode) {
            return currentSelector === (4 !== tNode.type || isProjectionMode ? tNode.value : NG_TEMPLATE_SELECTOR);
        }
        ...
        function selectIndexInternal(tView, lView, index, checkNoChangesMode) {
            if (!checkNoChangesMode) if (3 == (3 & lView[FLAGS])) {
        ...
        function refreshContentQueries(tView, lView) {
            ...
            if (null !== contentQueries) for (let i = 0; i < contentQueries.length; i += 2) {
                ...
                if (-1 !== directiveDefIdx) {
                    ...
                    setCurrentQueryIndex(contentQueries[i]), directiveDef.contentQueries(2, lView[directiveDefIdx], directiveDefIdx);
                }
            }
        }
        function createLView(parentLView, tView, context2, flags, host, tHostNode, rendererFactory, renderer, sanitizer, injector, embeddedViewInjector) {
            ...
            return lView[HOST] = host, lView[FLAGS] = 76 | flags, (null !== embeddedViewInjector || parentLView && 1024 & parentLView[FLAGS]) && (lView[FLAGS] |= 1024), 
            resetPreOrderHookFlags(lView), lView[PARENT] = lView[DECLARATION_VIEW] = parentLView, 
            lView[CONTEXT] = context2, lView[RENDERER_FACTORY] = rendererFactory || parentLView && parentLView[RENDERER_FACTORY], 
            ...
            lView[T_HOST] = tHostNode, lView[ID] = function getUniqueLViewId() {
                return uniqueIdCounter++;
            }(), lView[EMBEDDED_VIEW_INJECTOR] = embeddedViewInjector, lView[DECLARATION_COMPONENT_VIEW] = 2 == tView.type ? parentLView[DECLARATION_COMPONENT_VIEW] : lView, 
            lView;
        }
        function getOrCreateTNode(tView, index, type, name, attrs) {
            ...
            if (null === tNode) tNode = function createTNodeAtIndex(tView, index, type, name, attrs) {
                ...
                tNode.prev = currentTNode)), tNode;
            }(tView, index, type, name, attrs), function isInI18nBlock() {
                return instructionState.lFrame.inI18n;
            }() && (tNode.flags |= 32); else if (64 & tNode.type) {
        ...
        function refreshView(tView, lView, templateFn, context2) {
            ...
            if (128 != (128 & flags)) {
                ...
                try {
                    ...
                    if (function markTransplantedViewsForRefresh(lView) {
                        for (let lContainer = getFirstLContainer(lView); null !== lContainer; lContainer = getNextLContainer(lContainer)) {
                            ...
                            for (let i = 0; i < movedViews.length; i++) {
                                ...
                                512 & movedLView[FLAGS] || updateTransplantedViewCount(movedLView[PARENT], 1), movedLView[FLAGS] |= 512;
                            }
                        }
                    }(lView), function refreshEmbeddedViews(lView) {
                        for (let lContainer = getFirstLContainer(lView); null !== lContainer; lContainer = getNextLContainer(lContainer)) for (let i = CONTAINER_HEADER_OFFSET; i < lContainer.length; i++) {
                            const embeddedLView = lContainer[i], embeddedTView = embeddedLView[TVIEW];
                            viewAttachedToChangeDetector(embeddedLView) && refreshView(embeddedTView, embeddedLView, embeddedTView.template, embeddedLView[CONTEXT]);
                        }
                    ...
                    !0 === tView.firstUpdatePass && (tView.firstUpdatePass = !1), lView[FLAGS] &= -41, 
                    512 & lView[FLAGS] && (lView[FLAGS] &= -513, updateTransplantedViewCount(lView[PARENT], -1));
                } finally {
                    leaveView();
                }
            }
        ...
        function executeTemplate(tView, lView, templateFn, rf, context2) {
            ...
            try {
                setSelectedIndex(-1), isUpdatePhase && lView.length > HEADER_OFFSET && selectIndexInternal(tView, lView, HEADER_OFFSET, !1), 
                profiler(isUpdatePhase ? 2 : 0, context2), templateFn(rf, context2);
            } finally {
                setSelectedIndex(prevSelectedIndex), profiler(isUpdatePhase ? 3 : 1, context2);
            }
        }
        ...
        function createDirectivesInstances(tView, lView, tNode) {
            getBindingsEnabled() && (function instantiateAllDirectives(tView, lView, tNode, native) {
                ...
                isComponentHost(tNode) && function addComponentLogic(lView, hostTNode, def) {
                    const native = getNativeByTNode(hostTNode, lView), tView = getOrCreateComponentTView(def), rendererFactory = lView[RENDERER_FACTORY], componentView = addToViewTree(lView, createLView(lView, tView, null, def.onPush ? 32 : 16, native, hostTNode, rendererFactory, rendererFactory.createRenderer(native, def), null, null, null));
                    lView[hostTNode.index] = componentView;
                }(lView, tNode, tView.data[start + tNode.componentOffset]), tView.firstCreatePass || getOrCreateNodeInjectorForNode(tNode, lView), 
                attachPatchData(native, lView);
                const initialInputs = tNode.initialInputs;
                for (let i = start; i < end; i++) {
                    const def = tView.data[i], directive = getNodeInjectable(lView, tView, i, tNode);
                    attachPatchData(directive, lView), null !== initialInputs && setInputsFromAttrs(0, i - start, directive, def, 0, initialInputs), 
                    isComponentDef(def) && (getComponentLViewByIndex(tNode.index, lView)[CONTEXT] = getNodeInjectable(lView, tView, i, tNode));
                }
            }(tView, lView, tNode, getNativeByTNode(tNode, lView)), 64 == (64 & tNode.flags) && invokeDirectivesHostBindings(tView, lView, tNode));
        }
        ...
        function createTView(type, declTNode, templateFn, decls, vars, directives, pipes, viewQuery, schemas, constsOrFactory) {
            const bindingStartIndex = HEADER_OFFSET + decls, initialViewLength = bindingStartIndex + vars, blueprint = function createViewBlueprint(bindingStartIndex, initialViewLength) {
                const blueprint = [];
        ...
        function generatePropertyAliases(aliasMap, directiveIndex, propertyAliases, hostDirectiveAliasMap) {
            for (let publicName in aliasMap) if (aliasMap.hasOwnProperty(publicName)) {
                propertyAliases = null === propertyAliases ? {} : propertyAliases;
                const internalName = aliasMap[publicName];
                null === hostDirectiveAliasMap ? addPropertyAlias(propertyAliases, directiveIndex, publicName, internalName) : hostDirectiveAliasMap.hasOwnProperty(publicName) && addPropertyAlias(propertyAliases, directiveIndex, hostDirectiveAliasMap[publicName], internalName);
            }
            return propertyAliases;
        }
        function addPropertyAlias(propertyAliases, directiveIndex, publicName, internalName) {
            propertyAliases.hasOwnProperty(publicName) ? propertyAliases[publicName].push(directiveIndex, internalName) : propertyAliases[publicName] = [ directiveIndex, internalName ];
        }
        function markDirtyIfOnPush(lView, viewIndex) {
            const childComponentLView = getComponentLViewByIndex(viewIndex, lView);
            16 & childComponentLView[FLAGS] || (childComponentLView[FLAGS] |= 32);
        }
        function resolveDirectives(tView, lView, tNode, localRefs) {
            if (getBindingsEnabled()) {
                const exportsMap = null === localRefs ? null : {
                    "": -1
                }, matchResult = function findDirectiveDefMatches(tView, tNode) {
                    const registry = tView.directiveRegistry;
                    let matches = null, hostDirectiveDefs = null;
                    if (registry) for (let i = 0; i < registry.length; i++) {
                        const def = registry[i];
                        if (isNodeMatchingSelectorList(tNode, def.selectors, !1)) if (matches || (matches = []), 
                        isComponentDef(def)) if (null !== def.findHostDirectiveDefs) {
                            const hostDirectiveMatches = [];
                            hostDirectiveDefs = hostDirectiveDefs || new Map, def.findHostDirectiveDefs(def, hostDirectiveMatches, hostDirectiveDefs), 
                            matches.unshift(...hostDirectiveMatches, def), markAsComponentHost(tView, tNode, hostDirectiveMatches.length);
                        } else matches.unshift(def), markAsComponentHost(tView, tNode, 0); else hostDirectiveDefs = hostDirectiveDefs || new Map, 
                        def.findHostDirectiveDefs?.(def, matches, hostDirectiveDefs), matches.push(def);
                    }
                    return null === matches ? null : [ matches, hostDirectiveDefs ];
                }(tView, tNode);
                let directiveDefs, hostDirectiveDefs;
                null === matchResult ? directiveDefs = hostDirectiveDefs = null : [directiveDefs, hostDirectiveDefs] = matchResult, 
                null !== directiveDefs && initializeDirectives(tView, lView, tNode, directiveDefs, exportsMap, hostDirectiveDefs), 
        ...
        function initializeDirectives(tView, lView, tNode, directives, exportsMap, hostDirectiveDefs) {
            ...
            }(tNode, tView.data.length, directives.length);
            for (let i = 0; i < directives.length; i++) {
                const def = directives[i];
                def.providersResolver && def.providersResolver(def);
            }
            let preOrderHooksFound = !1, preOrderCheckHooksFound = !1, directiveIdx = allocExpando(tView, lView, directives.length, null);
            for (let i = 0; i < directives.length; i++) {
                const def = directives[i];
                ...
                saveNameToExportMap(directiveIdx, def, exportsMap), null !== def.contentQueries && (tNode.flags |= 4), 
                (null !== def.hostBindings || null !== def.hostAttrs || 0 !== def.hostVars) && (tNode.flags |= 64);
            ...
            !function initializeInputAndOutputAliases(tView, tNode, hostDirectiveDefinitionMap) {
                ...
                for (let directiveIndex = tNode.directiveStart; directiveIndex < end; directiveIndex++) {
                    const directiveDef = tViewData[directiveIndex], aliasData = hostDirectiveDefinitionMap ? hostDirectiveDefinitionMap.get(directiveDef) : null, aliasedOutputs = aliasData ? aliasData.outputs : null;
                    inputsStore = generatePropertyAliases(directiveDef.inputs, directiveIndex, inputsStore, aliasData ? aliasData.inputs : null), 
                    outputsStore = generatePropertyAliases(directiveDef.outputs, directiveIndex, outputsStore, aliasedOutputs);
                    const initialInputs = null === inputsStore || null === tNodeAttrs || isInlineTemplate(tNode) ? null : generateInitialInputs(inputsStore, directiveIndex, tNodeAttrs);
                    inputsFromAttrs.push(initialInputs);
                }
                null !== inputsStore && (inputsStore.hasOwnProperty("class") && (tNode.flags |= 8), 
                inputsStore.hasOwnProperty("style") && (tNode.flags |= 16)), tNode.initialInputs = inputsFromAttrs, 
                tNode.inputs = inputsStore, tNode.outputs = outputsStore;
            }(tView, tNode, hostDirectiveDefs);
        }
        ...
        function markAsComponentHost(tView, hostTNode, componentOffset) {
            hostTNode.componentOffset = componentOffset, (tView.components || (tView.components = [])).push(hostTNode.index);
        }
        ...
        function createLContainer(hostNative, currentView, native, tNode) {
            return [ hostNative, !0, !1, currentView, null, 0, tNode, native, null, null ];
        }
        function refreshComponent(hostLView, componentHostIdx) {
            const componentView = getComponentLViewByIndex(componentHostIdx, hostLView);
            if (viewAttachedToChangeDetector(componentView)) {
                const tView = componentView[TVIEW];
                48 & componentView[FLAGS] ? refreshView(tView, componentView, tView.template, componentView[CONTEXT]) : componentView[TRANSPLANTED_VIEWS_TO_REFRESH] > 0 && refreshContainsDirtyView(componentView);
            }
        }
        function refreshContainsDirtyView(lView) {
            for (let lContainer = getFirstLContainer(lView); null !== lContainer; lContainer = getNextLContainer(lContainer)) for (let i = CONTAINER_HEADER_OFFSET; i < lContainer.length; i++) {
                const embeddedLView = lContainer[i];
                if (viewAttachedToChangeDetector(embeddedLView)) if (512 & embeddedLView[FLAGS]) {
                    const embeddedTView = embeddedLView[TVIEW];
                    refreshView(embeddedTView, embeddedLView, embeddedTView.template, embeddedLView[CONTEXT]);
                } else embeddedLView[TRANSPLANTED_VIEWS_TO_REFRESH] > 0 && refreshContainsDirtyView(embeddedLView);
            }
            const components = lView[TVIEW].components;
            if (null !== components) for (let i = 0; i < components.length; i++) {
                const componentView = getComponentLViewByIndex(components[i], lView);
                viewAttachedToChangeDetector(componentView) && componentView[TRANSPLANTED_VIEWS_TO_REFRESH] > 0 && refreshContainsDirtyView(componentView);
            }
        }
        function renderComponent(hostLView, componentHostIdx) {
            const componentView = getComponentLViewByIndex(componentHostIdx, hostLView), componentTView = componentView[TVIEW];
            (function syncViewWithBlueprint(tView, lView) {
                for (let i = lView.length; i < tView.blueprint.length; i++) lView.push(tView.blueprint[i]);
            })(componentTView, componentView), renderView(componentTView, componentView, componentView[CONTEXT]);
        }
        ...
        function markViewDirty(lView) {
            for (;lView; ) {
                ...
                if (0 != (256 & lView[FLAGS]) && !parent) return lView;
                lView = parent;
            }
            return null;
        }
        ...
        function handleError(lView, error) {
            const injector = lView[INJECTOR$1], errorHandler2 = injector ? injector.get(ErrorHandler, null) : null;
        ...
        function collectNativeNodes(tView, lView, tNode, result, isProjection = !1) {
            for (;null !== tNode; ) {
                ...
                if (null !== lNode && result.push(unwrapRNode(lNode)), isLContainer(lNode)) for (let i = CONTAINER_HEADER_OFFSET; i < lNode.length; i++) {
                    ...
                    null !== lViewFirstChildTNode && collectNativeNodes(lViewInAContainer[TVIEW], lViewInAContainer, lViewFirstChildTNode, result);
                }
                const tNodeType = tNode.type;
                if (8 & tNodeType) collectNativeNodes(tView, lView, tNode.child, result); else if (32 & tNodeType) {
                    ...
                    if (Array.isArray(nodesInSlot)) result.push(...nodesInSlot); else {
                        ...
                        collectNativeNodes(parentView[TVIEW], parentView, nodesInSlot, result, !0);
                    }
                }
                tNode = isProjection ? tNode.projectionNext : tNode.next;
            }
        ...
        class ViewRef$1 {
            get rootNodes() {
                const lView = this._lView, tView = lView[TVIEW];
            ...
            get destroyed() {
                return 128 == (128 & this._lView[FLAGS]);
            }
            destroy() {
                if (this._appRef) this._appRef.detachView(this); else if (this._attachedToViewContainer) {
                    const parent = this._lView[PARENT];
                    if (isLContainer(parent)) {
                        const viewRefs = parent[VIEW_REFS], index = viewRefs ? viewRefs.indexOf(this) : -1;
            ...
            onDestroy(callback) {
                ...
                }(this._lView[TVIEW], this._lView, null, callback);
            }
            markForCheck() {
                markViewDirty(this._cdRefInjectingView || this._lView);
            }
            detach() {
                this._lView[FLAGS] &= -65;
            }
            reattach() {
        ...
        class RootViewRef extends ViewRef$1 {
            ...
            detectChanges() {
                ...
                detectChangesInternal(lView[TVIEW], lView, lView[CONTEXT], !1);
            }
            checkNoChanges() {}
            get context() {
                return null;
        ...
        class ComponentFactory extends ComponentFactory$1 {
            ...
            create(injector, projectableNodes, rootSelectorOrNode, environmentInjector) {
                ...
                try {
                    ...
                    let rootDirectives, hostDirectiveDefs = null;
                    rootComponentDef.findHostDirectiveDefs ? (rootDirectives = [], hostDirectiveDefs = new Map, 
                    rootComponentDef.findHostDirectiveDefs(rootComponentDef, rootDirectives, hostDirectiveDefs), 
                    rootDirectives.push(rootComponentDef)) : rootDirectives = [ rootComponentDef ];
                    const componentView = function createRootComponentView(tNode, rNode, rootComponentDef, rootDirectives, rootView, rendererFactory, hostRenderer, sanitizer) {
                        ...
                        !function applyRootComponentStyling(rootDirectives, tNode, rNode, hostRenderer) {
                            ...
                            null !== rNode && setupStaticAttributes(hostRenderer, rNode, tNode));
                        }(rootDirectives, tNode, rNode, hostRenderer);
                        const viewRenderer = rendererFactory.createRenderer(rNode, rootComponentDef), componentView = createLView(rootView, getOrCreateComponentTView(rootComponentDef), null, rootComponentDef.onPush ? 32 : 16, rootView[tNode.index], tNode, rendererFactory, viewRenderer, sanitizer || null, null, null);
                        return tView.firstCreatePass && markAsComponentHost(tView, tNode, rootDirectives.length - 1), 
                        addToViewTree(rootView, componentView), rootView[tNode.index] = componentView;
                    }(function createRootComponentTNode(lView, rNode) {
                        const tView = lView[TVIEW], index = HEADER_OFFSET;
                        return lView[index] = rNode, getOrCreateTNode(tView, index, 2, "#host", null);
                    }(rootLView, hostRNode), hostRNode, rootComponentDef, rootDirectives, rootLView, rendererFactory, hostRenderer);
                    tElementNode = getTNode(rootTView, HEADER_OFFSET), hostRNode && function setRootNodeAttributes(hostRenderer, componentDef, hostRNode, rootSelectorOrNode) {
                        if (rootSelectorOrNode) setUpAttributes(hostRenderer, hostRNode, [ "ng-version", VERSION.full ]); else {
                    ...
                    }(hostRenderer, rootComponentDef, hostRNode, rootSelectorOrNode), void 0 !== projectableNodes && function projectNodes(tNode, ngContentSelectors, projectableNodes) {
                        const projection = tNode.projection = [];
                        for (let i = 0; i < ngContentSelectors.length; i++) {
                            const nodesforSlot = projectableNodes[i];
                            projection.push(null != nodesforSlot ? Array.from(nodesforSlot) : null);
                        }
                    }(tElementNode, this.ngContentSelectors, projectableNodes), component = function createRootComponent(componentView, rootComponentDef, rootDirectives, hostDirectiveDefs, rootLView, hostFeatures) {
                        const rootTNode = getCurrentTNode(), tView = rootLView[TVIEW], native = getNativeByTNode(rootTNode, rootLView);
                        initializeDirectives(tView, rootLView, rootTNode, rootDirectives, null, hostDirectiveDefs);
                        for (let i = 0; i < rootDirectives.length; i++) attachPatchData(getNodeInjectable(rootLView, tView, rootTNode.directiveStart + i, rootTNode), rootLView);
                        invokeDirectivesHostBindings(tView, rootLView, rootTNode), native && attachPatchData(native, rootLView);
                        const component = getNodeInjectable(rootLView, tView, rootTNode.directiveStart + rootTNode.componentOffset, rootTNode);
                        if (componentView[CONTEXT] = rootLView[CONTEXT] = component, null !== hostFeatures) for (const feature of hostFeatures) feature(component, rootComponentDef);
                        return executeContentQueries(tView, rootTNode, componentView), component;
                    }(componentView, rootComponentDef, rootDirectives, hostDirectiveDefs, rootLView, [ LifecycleHooksFeature ]), 
        ...
        class ComponentRef extends ComponentRef$1 {
            ...
            setInput(name, value) {
                ...
                if (null !== inputData && (dataValue = inputData[name])) {
                    ...
                    setInputsForProperty(lView[TVIEW], lView, dataValue, name, value), markDirtyIfOnPush(lView, this._tNode.index);
                }
            }
            get injector() {
        ...
        function isListLikeIterable(obj) {
            ...
            }(obj) && (Array.isArray(obj) || !(obj instanceof Map) && Symbol.iterator in obj);
        }
        ...
        function ɵɵtemplate(index, templateFn, decls, vars, tagName, attrsIndex, localRefsIndex, localRefExtractor) {
            ...
            const comment = lView[RENDERER].createComment("");
        ...
        function ɵɵelementStart(index, name, attrsIndex, localRefsIndex) {
            ...
            }(adjustedIndex, tView, lView, name, attrsIndex, localRefsIndex) : tView.data[adjustedIndex], native = lView[adjustedIndex] = createElementNode(renderer, name, function getNamespace$1() {
                return instructionState.lFrame.currentNamespace;
            }()), hasDirectives = isDirectiveHost(tNode);
            return setCurrentTNode(tNode, !0), setupStaticAttributes(renderer, native, tNode), 
            32 != (32 & tNode.flags) && appendChild(tView, lView, native, tNode), 0 === function getElementDepthCount() {
        ...
        function executeListenerWithErrorHandling(lView, context2, listenerFn, e) {
            try {
                return profiler(6, context2, listenerFn), !1 !== listenerFn(e);
            } catch (error) {
        ...
        function wrapListener(tNode, lView, context2, listenerFn, wrapWithPreventDefault) {
            return function wrapListenerIn_markDirtyAndPreventDefault(e) {
                ...
                markViewDirty(tNode.componentOffset > -1 ? getComponentLViewByIndex(tNode.index, lView) : lView);
                let result = executeListenerWithErrorHandling(lView, context2, listenerFn, e), nextListenerFn = wrapListenerIn_markDirtyAndPreventDefault.__ngNextListenerFn__;
                for (;nextListenerFn; ) result = executeListenerWithErrorHandling(lView, context2, nextListenerFn, e) && result, 
        ...
        function ɵɵnextContext(level = 1) {
            return function nextContextImpl(level) {
                return (instructionState.lFrame.contextLView = function walkUpViews(nestingLevel, currentView) {
                    ...
                    return currentView;
                }(level, instructionState.lFrame.contextLView))[CONTEXT];
            }(level);
        }
        ...
        function ɵɵclassProp(className, value) {
            return function checkStylingProperty(prop, value, suffix, isClassBased) {
                ...
                tView.firstUpdatePass && function stylingFirstUpdatePass(tView, tStylingKey, bindingIndex, isClassBased) {
                    ...
                    if (null === tData[bindingIndex + 1]) {
                        ...
                        (function hasStylingInputShadow(tNode, isClassBased) {
                            return 0 != (tNode.flags & (isClassBased ? 8 : 16));
                        ...
                        tStylingKey = function wrapInStaticStylingKey(tData, tNode, stylingKey, isClassBased) {
                            ...
                            if (Array.isArray(tStylingKeyWithStatic) ? (tStylingKey = tStylingKeyWithStatic[1], 
                            (null === tStylingKey || keyValueArrayIndexOf(tStylingKeyWithStatic, tStylingKey) > 0) && (isKeyDuplicateOfStatic = !0)) : tStylingKey = tStylingKeyWithStatic, 
                            isHostBinding) if (0 !== tmplTail) {
                ...
                }(tView, tView.data[getSelectedIndex()], lView, lView[RENDERER], prop, lView[bindingIndex + 1] = function normalizeSuffix(value, suffix) {
                    return null == value || "" === value || ("string" == typeof suffix ? value += suffix : "object" == typeof value && (value = stringify(function unwrapSafeValue(value) {
        ...
        function resolveProvider(provider, tInjectables, lInjectablesBlueprint, isComponent, isViewProvider) {
            if (provider = resolveForwardRef(provider), Array.isArray(provider)) for (let i = 0; i < provider.length; i++) resolveProvider(provider[i], tInjectables, lInjectablesBlueprint, isComponent, isViewProvider); else {
                ...
                if (isTypeProvider(provider) || !provider.multi) {
                    ...
                    const existingProvidersFactoryIndex = indexOf(token, tInjectables, beginIndex + cptViewProvidersCount, endIndex), existingViewProvidersFactoryIndex = indexOf(token, tInjectables, beginIndex, beginIndex + cptViewProvidersCount), doesViewProvidersFactoryExist = existingViewProvidersFactoryIndex >= 0 && lInjectablesBlueprint[existingViewProvidersFactoryIndex];
        ...
        function multiViewProvidersFactoryResolver(_, tData, lView, tNode) {
            ...
            if (this.providerFactory) {
                const componentCount = this.providerFactory.componentProviders, multiProviders = getNodeInjectable(lView, lView[TVIEW], this.providerFactory.index, tNode);
        ...
        const ViewEngineTemplateRef = TemplateRef, R3TemplateRef = class extends ViewEngineTemplateRef {
            ...
            createEmbeddedView(context2, injector) {
                const embeddedTView = this._declarationTContainer.tView, embeddedLView = createLView(this._declarationLView, embeddedTView, context2, 16, null, embeddedTView.declTNode, null, null, null, null, injector || null);
                embeddedLView[DECLARATION_LCONTAINER] = this._declarationLView[this._declarationTContainer.index];
                const declarationViewLQueries = this._declarationLView[QUERIES];
                return null !== declarationViewLQueries && (embeddedLView[QUERIES] = declarationViewLQueries.createEmbeddedView(embeddedTView)), 
        ...
        const VE_ViewContainerRef = ViewContainerRef, R3ViewContainerRef = class extends VE_ViewContainerRef {
            ...
            get parentInjector() {
                ...
                if (hasParentInjector(parentLocation)) {
                    ...
                    return new NodeInjector(parentView[TVIEW].data[injectorIndex + 8], parentView);
                }
            ...
            insert(viewRef, index) {
                const lView = viewRef._lView, tView = lView[TVIEW];
                if (function viewAttachedToContainer(view) {
                    return isLContainer(view[PARENT]);
                }(lView)) {
                    const prevIdx = this.indexOf(viewRef);
                    if (-1 !== prevIdx) this.detach(prevIdx); else {
                        const prevLContainer = lView[PARENT], prevVCRef = new R3ViewContainerRef(prevLContainer, prevLContainer[T_HOST], prevLContainer[PARENT]);
                ...
                !function insertView(tView, lView, lContainer, index) {
                    ...
                    null !== declarationLContainer && lContainer !== declarationLContainer && function trackMovedView(declarationContainer, lView) {
                        ...
                        lView[DECLARATION_COMPONENT_VIEW] !== lView[PARENT][PARENT][DECLARATION_COMPONENT_VIEW] && (declarationContainer[HAS_TRANSPLANTED_VIEWS] = !0), 
                        null === movedViews ? declarationContainer[MOVED_VIEWS] = [ lView ] : movedViews.push(lView);
                    }(declarationLContainer, lView);
                    const lQueries = lView[QUERIES];
                    null !== lQueries && lQueries.insertView(tView), lView[FLAGS] |= 64;
                }(tView, lView, lContainer, adjustedIdx);
                const beforeNode = getBeforeNodeForView(adjustedIdx, lContainer), renderer = lView[RENDERER], parentRNode = nativeParentNode(renderer, lContainer[NATIVE]);
                return null !== parentRNode && function addViewToContainer(tView, parentTNode, renderer, lView, parentNativeNode, beforeNode) {
                    lView[HOST] = parentNativeNode, lView[T_HOST] = parentTNode, applyView(tView, lView, renderer, 1, parentNativeNode, beforeNode);
                }(tView, lContainer[T_HOST], renderer, lView, parentRNode, beforeNode), viewRef.attachToViewContainerRef(), 
        ...
        class core_NgZone {
            constructor({enableLongStackTrace = !1, shouldCoalesceEventChangeDetection = !1, shouldCoalesceRunChangeDetection = !1}) {
                ...
                this.shouldCoalesceEventChangeDetection = !shouldCoalesceRunChangeDetection && shouldCoalesceEventChangeDetection, 
                this.shouldCoalesceRunChangeDetection = shouldCoalesceRunChangeDetection, this.lastRequestAnimationFrameId = -1, 
                this.nativeRequestAnimationFrame = function getNativeRequestAnimationFrame() {
        ...
        const ALLOW_MULTIPLE_PLATFORMS = new InjectionToken("AllowMultipleToken"), PLATFORM_DESTROY_LISTENERS = new InjectionToken("PlatformDestroyListeners"), APP_BOOTSTRAP_LISTENER = new InjectionToken("appBootstrapListener");
            ...
            class ApplicationRef2 {
                ...
                bootstrap(componentOrFactory, rootSelectorOrNode) {
                    ...
                    if (!this._injector.get(ApplicationInitStatus).done) {
                        !isComponentFactory && function isStandalone(type) {
                            const def = getComponentDef(type) || getDirectiveDef(type) || getPipeDef$1(type);
                            return null !== def && def.standalone;
                        }(componentOrFactory);
                        throw new RuntimeError(405, false);
                    }
                ...
                _loadComponent(componentRef) {
                    ...
                    listeners.push(...this._bootstrapListeners), listeners.forEach(listener => listener(componentRef));
                }
                ngOnDestroy() {
        ...
        function injectChangeDetectorRef(flags) {
            return function createViewRef(tNode, lView, isPipe) {
                ...
                return 47 & tNode.type ? new ViewRef$1(lView[DECLARATION_COMPONENT_VIEW], lView) : null;
        ...
        let EventManager = (() => {
            class EventManager2 {
                constructor(plugins, _zone) {
                    this._zone = _zone, this._eventNameToPlugin = new Map, plugins.forEach(plugin => {
                        plugin.manager = this;
        ...
        let SharedStylesHost = (() => {
            class SharedStylesHost2 {
                ...
                addStyles(styles) {
                    for (const style of styles) 1 === this.changeUsageCount(style, 1) && this.onStyleAdded(style);
                }
                removeStyles(styles) {
                    for (const style of styles) 0 === this.changeUsageCount(style, -1) && this.onStyleRemoved(style);
                }
                ...
                changeUsageCount(style, delta) {
                    ...
                    return usage += delta, usage > 0 ? map2.set(style, usage) : map2.delete(style), 
                    usage;
                }
                ngOnDestroy() {
                    for (const style of this.getAllStyles()) this.onStyleRemoved(style);
            ...
            class DomSharedStylesHost2 extends SharedStylesHost {
                constructor(doc) {
                    super(), this.doc = doc, this.styleRef = new Map, this.hostNodes = new Set, this.resetHostNodes();
                }
                onStyleAdded(style) {
                    for (const host of this.hostNodes) this.addStyleToHost(host, style);
                }
                onStyleRemoved(style) {
                    const styleRef = this.styleRef;
                    styleRef.get(style)?.forEach(e => e.remove()), styleRef.delete(style);
                }
                ngOnDestroy() {
                ...
                addHost(hostNode) {
                    ...
                    for (const style of this.getAllStyles()) this.addStyleToHost(hostNode, style);
                }
                removeHost(hostNode) {
                ...
                addStyleToHost(host, style) {
                    ...
                    styleEl.textContent = style, host.appendChild(styleEl);
                    const styleElRef = this.styleRef.get(style);
                    styleElRef ? styleElRef.push(styleEl) : this.styleRef.set(style, [ styleEl ]);
                }
                resetHostNodes() {
        ...
        }, COMPONENT_REGEX = /%COMP%/g, REMOVE_STYLES_ON_COMPONENT_DESTROY = new InjectionToken("RemoveStylesOnCompDestory", {
            providedIn: "root",
            factory: () => !1
        });
        ...
        let DomRendererFactory2 = (() => {
            class DomRendererFactory22 {
                constructor(eventManager, sharedStylesHost, appId, removeStylesOnCompDestory) {
                    ...
                    this.removeStylesOnCompDestory = removeStylesOnCompDestory, this.rendererByCompId = new Map, 
                    this.defaultRenderer = new DefaultDomRenderer2(eventManager);
                }
                createRenderer(element, type) {
                    if (!element || !type) return this.defaultRenderer;
                    const renderer = this.getOrCreateRenderer(element, type);
                    return renderer instanceof EmulatedEncapsulationDomRenderer2 ? renderer.applyToHost(element) : renderer instanceof NoneEncapsulationDomRenderer && renderer.applyStyles(), 
                    renderer;
                }
                getOrCreateRenderer(element, type) {
                    ...
                    if (!renderer) {
                        const eventManager = this.eventManager, sharedStylesHost = this.sharedStylesHost, removeStylesOnCompDestory = this.removeStylesOnCompDestory;
                        ...
                        renderer.onDestroy = () => rendererByCompId.delete(type.id), rendererByCompId.set(type.id, renderer);
                    }
                    return renderer;
                }
                ngOnDestroy() {
        ...
        class ShadowDomRenderer extends DefaultDomRenderer2 {
            constructor(eventManager, sharedStylesHost, hostEl, component) {
                ...
                for (const style of styles) {
                    ...
                    styleEl.textContent = style, this.shadowRoot.appendChild(styleEl);
                }
            }
        ...
        class NoneEncapsulationDomRenderer extends DefaultDomRenderer2 {
            constructor(eventManager, sharedStylesHost, component, removeStylesOnCompDestory, compId = component.id) {
                super(eventManager), this.sharedStylesHost = sharedStylesHost, this.removeStylesOnCompDestory = removeStylesOnCompDestory, 
                this.rendererUsageCount = 0, this.styles = flattenStyles(compId, component.styles);
            }
            applyStyles() {
            ...
            destroy() {
                ...
                this.rendererUsageCount--, 0 === this.rendererUsageCount && this.onDestroy?.());
            }
        }
        class EmulatedEncapsulationDomRenderer2 extends NoneEncapsulationDomRenderer {
            constructor(eventManager, sharedStylesHost, component, appId, removeStylesOnCompDestory) {
                ...
                this.contentAttr = function shimContentAttribute(componentShortId) {
                    return "_ngcontent-%COMP%".replace(COMPONENT_REGEX, componentShortId);
                }(compId), this.hostAttr = function shimHostAttribute(componentShortId) {
                    return "_nghost-%COMP%".replace(COMPONENT_REGEX, componentShortId);
                }(compId);
            }
            applyToHost(element) {
                this.applyStyles(), this.setAttribute(element, this.hostAttr, "");
            }
        ...
        const platformBrowser = createPlatformFactory(platformCore, "browser", [ {
            ...
            deps: [ EventManager, DomSharedStylesHost, core_APP_ID, REMOVE_STYLES_ON_COMPONENT_DESTROY ]
        }, {
        ...
        const HTTP_INTERCEPTORS = new InjectionToken("HTTP_INTERCEPTORS"), HTTP_INTERCEPTOR_FNS = new InjectionToken("HTTP_INTERCEPTOR_FNS");
        function legacyInterceptorFnFactory() {
            let chain = null;
            return (req, handler) => (null === chain && (chain = (fesm2020_core_inject(HTTP_INTERCEPTORS, {
                optional: !0
            }) ?? []).reduceRight(adaptLegacyInterceptorToChain, interceptorChainEndFn)), chain(req, handler));
        }
        let HttpInterceptorHandler = (() => {
            class HttpInterceptorHandler2 extends HttpHandler {
                constructor(backend, injector) {
                    super(), this.backend = backend, this.injector = injector, this.chain = null;
                }
                handle(initialRequest) {
                    if (null === this.chain) {
                        const dedupedInterceptorFns = Array.from(new Set(this.injector.get(HTTP_INTERCEPTOR_FNS)));
                        this.chain = dedupedInterceptorFns.reduceRight((nextSequencedFn, interceptorFn) => function chainedInterceptorFn(chainTailFn, interceptorFn, injector) {
                            return (initialRequest, finalHandlerFn) => injector.runInContext(() => interceptorFn(initialRequest, downstreamRequest => chainTailFn(downstreamRequest, finalHandlerFn)));
                        }(nextSequencedFn, interceptorFn, this.injector), interceptorChainEndFn);
                    }
        ...
        function xsrfInterceptorFn(req, next) {
            ...
            if (!fesm2020_core_inject(XSRF_ENABLED) || "GET" === req.method || "HEAD" === req.method || lcUrl.startsWith("http://") || lcUrl.startsWith("https://")) return next(req);
            const token = fesm2020_core_inject(HttpXsrfTokenExtractor).getToken(), headerName = fesm2020_core_inject(XSRF_HEADER_NAME);
            return null != token && !req.headers.has(headerName) && (req = req.clone({
                headers: req.headers.set(headerName, token)
            })), next(req);
        }
        var HttpFeatureKind = (() => ((HttpFeatureKind = HttpFeatureKind || {})[HttpFeatureKind.Interceptors = 0] = "Interceptors", 
        HttpFeatureKind[HttpFeatureKind.LegacyInterceptors = 1] = "LegacyInterceptors", 
        HttpFeatureKind[HttpFeatureKind.CustomXsrfConfiguration = 2] = "CustomXsrfConfiguration", 
        HttpFeatureKind[HttpFeatureKind.NoXsrfProtection = 3] = "NoXsrfProtection", HttpFeatureKind[HttpFeatureKind.JsonpSupport = 4] = "JsonpSupport", 
        ...
        function toObservable(value) {
            ...
            if (forms_NG_DEV_MODE$1 && !isObservable(obs)) {
                ...
                throw "object" == typeof value && (errorMessage += " Are you using a synchronous validator where an async validator is expected?"), 
                new RuntimeError(-1101, errorMessage);
            }
            return obs;
        }
        ...
        class AbstractControl {
            constructor(validators, asyncValidators) {
                ...
                this._onDisabledChange = [], this._assignValidators(validators), this._assignAsyncValidators(asyncValidators);
            }
            get validator() {
            ...
            _assignValidators(validators) {
                ...
                this._composedValidatorFn = function coerceToValidator(validator) {
                    return Array.isArray(validator) ? composeValidators(validator) : validator || null;
                }(this._rawValidators);
            }
            _assignAsyncValidators(validators) {
                this._rawAsyncValidators = Array.isArray(validators) ? validators.slice() : validators, 
                this._composedAsyncValidatorFn = function coerceToAsyncValidator(asyncValidator) {
        ...
        function setUpControl(control, dir, callSetDisabledState = setDisabledStateDefault) {
            ...
            })(control, dir), dir.valueAccessor.writeValue(control.value), (control.disabled || "always" === callSetDisabledState) && dir.valueAccessor.setDisabledState?.(control.disabled), 
        ...
        const FormControl = class extends AbstractControl {
        ...
        let NgModel = (() => {
            class NgModel2 extends NgControl {
                constructor(parent, validators, asyncValidators, valueAccessors, _changeDetectorRef, callSetDisabledState) {
                    super(), this._changeDetectorRef = _changeDetectorRef, this.callSetDisabledState = callSetDisabledState, 
                    this.control = new FormControl, this._registered = !1, this.update = new core_EventEmitter, 
                    this._parent = parent, this._setValidators(validators), this._setAsyncValidators(asyncValidators), 
                ...
                _setUpStandalone() {
                    setUpControl(this.control, this, this.callSetDisabledState), this.control.updateValueAndValidity({
                        emitEvent: !1
                    });
                }
                _checkForErrors() {
